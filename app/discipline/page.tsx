...
export default async function DisciplinesPage({ searchParams }: DisciplinesPageProps) {
    ...
    // Group disciplines by learning type
    const licentaDisciplines = filteredDisciplines.filter(
        (d) => d.learningType?.learningCycle === "Licenta"
    )

    console.log(licentaDisciplines)

    const masterDisciplines = filteredDisciplines.filter(
        (d) => d.learningType?.learningCycle === "Master"
    )

    console.log(masterDisciplines)

    const hasLicenta = licentaDisciplines.length > 0
    const hasMaster = masterDisciplines.length > 0
    const hasAnyResults = hasLicenta || hasMaster
    const hasNoDisciplines = disciplines.length === 0

    return (
        <div className="flex flex-col overflow-hidden h-full">
            <div className="flex flex-col flex-shrink-0 items-center bg-primary-200 border-b border-primary-300 p-6">
                <div className="flex justify-between items-end gap-1 w-full max-w-7xl">
                    <div className="flex flex-col gap-2">
                        <H1 className="text-left text-2xl">Discipline</H1>
                        <P className="text-base [&:not(:first-child)]:mt-0">Gestionează disciplinele din platformă.</P>
                    </div>
                ...
                <div className="flex flex-col items-center py-8 px-6">
                    <div className="flex flex-col gap-8 w-full max-w-7xl">
                        <Suspense fallback={"Loading..."}>
                            ...
                            {!hasAnyResults && searchQuery && !hasNoDisciplines && (
                                <Empty>
                                    ...
                                    <EmptyContent>
                                        ...
                                        <EmptyDescription>
                                            Nu au fost găsite discipline care să corespundă criteriilor "{searchQuery}"
                                        </EmptyDescription>
                                    </EmptyContent>
                                </Empty>
                            )}
                            ...
                            {hasNoDisciplines && (
                                <Empty>
                                    ...
                                    <EmptyContent>
                                        ...
                                        <EmptyDescription>
                                            Nu ai adăugat încă nicio disciplină. Creează prima disciplină pentru a începe organizarea.
                                        </EmptyDescription>
                                        <AdminOnlyServer>
                                ...
                                <section className="flex flex-col gap-4">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <H2 className="text-xl pb-0">Licență</H2>
                                            <span className="text-sm text-primary-500 bg-primary-200 px-2 py-0.5 rounded-full">
                                                {licentaDisciplines.length} {licentaDisciplines.length === 1 ? 'disciplină' : 'discipline'}
                                            </span>
                                        </div>
                                    </div>
                                    <ul className="grid grid-cols-[repeat(auto-fill,minmax(320px,1fr))] auto-rows-min gap-6 w-full">
                                        {licentaDisciplines.map((discipline) => (
                                            <DisciplineCard
                                                key={discipline.id}
                                                discipline={discipline}
                                            />
                                        ))}
                                        {/* Add card - only show if not searching */}
                                        <AdminOnlyServer>
                                            {!searchQuery && (
                                                <li className="min-h-[120px]">
                                                    <CreateDisciplineModal
                                                        trigger={
                                                            <Card className="bg-primary-100 h-full p-0 hover:bg-green-500/10 hover:border-green-500 transition-colors cursor-pointer border-dashed">
                                                                <CardContent className="h-full flex items-center justify-center p-0">
                                                                    <Button className="flex-col py-4 px-6 w-full h-full text-base text-green-600" variant="text" size="L">
                                                                        <CirclePlus className="size-6" /> Adaugă disciplină
                                                                    </Button>
                                                                </CardContent>
                                                            </Card>
                                                        }
                                                        learningTypes={learningTypes}
                                                        teachers={teachers}
                                                        defaultLearningTypeId={learningTypes[0].id}
                                                    />
                                                </li>
                                            )}
                                        </AdminOnlyServer>
                                ...
                                <section className="flex flex-col gap-4">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <H2 className="text-xl pb-0">Master</H2>
                                            <span className="text-sm text-primary-500 bg-primary-200 px-2 py-0.5 rounded-full">
                                                {masterDisciplines.length} {masterDisciplines.length === 1 ? 'disciplină' : 'discipline'}
                                            </span>
                                        </div>
                                    </div>
                                    <ul className="grid grid-cols-[repeat(auto-fill,minmax(320px,1fr))] auto-rows-min gap-6 w-full">
                                        {masterDisciplines.map((discipline) => (
                                            <DisciplineCard
                                                key={discipline.id}
                                                discipline={discipline}
                                            />
                                        ...
                                        <AdminOnlyServer>
                                            {!searchQuery && (
                                                <li className="min-h-[120px]">
                                                    <CreateDisciplineModal
                                                        trigger={
                                                            <Card className="bg-primary-100 h-full p-0 hover:bg-orange-400/10 hover:border-orange-400 transition-colors cursor-pointer border-dashed">
                                                                <CardContent className="h-full flex items-center justify-center p-0">
                                                                    <Button className="flex-col py-4 px-6 w-full h-full text-base text-orange-500" variant="text" size="L">
                                                                        <CirclePlus className="size-6" /> Adaugă disciplină
                                                                    </Button>
                                                                </CardContent>
                                                            </Card>
                                                        ...
                                                        learningTypes={learningTypes}
                                                        teachers={teachers}
                                                        defaultLearningTypeId={learningTypes[1].id}
                                                    />
                                                </li>
                                            )}
                                        </AdminOnlyServer>
                            ...
                            {/* Show both add buttons when no disciplines exist but we have learning types */}
                            {!hasNoDisciplines && !hasAnyResults && !searchQuery && (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <section className="flex flex-col gap-4">
                                        ...
                                        <AdminOnlyServer>
                                            <CreateDisciplineModal
                                                trigger={
                                                    <Card className="bg-primary-100 h-32 p-0 hover:bg-green-500/10 hover:border-green-500 transition-colors cursor-pointer border-dashed">
                                                        <CardContent className="h-full flex items-center justify-center p-0">
                                                            <Button className="flex-col py-4 px-6 w-full h-full text-base text-green-600" variant="text" size="L">
                                                                <CirclePlus className="size-6" /> Adaugă disciplină
                                                            </Button>
                                    ...
                                    <section className="flex flex-col gap-4">
                                        ...
                                        <AdminOnlyServer>
                                            <CreateDisciplineModal
                                                trigger={
                                                    <Card className="bg-primary-100 h-32 p-0 hover:bg-orange-400/10 hover:border-orange-400 transition-colors cursor-pointer border-dashed">
                                                        <CardContent className="h-full flex items-center justify-center p-0">
                                                            <Button className="flex-col py-4 px-6 w-full h-full text-base text-orange-500" variant="text" size="L">
                                                                <CirclePlus className="size-6" /> Adaugă disciplină
                                                            </Button>
