...
export default async function YearsPage() {
    const [years, learningTypes] = await Promise.all([
        getAllYears(),
        getAllLearningTypes()
    ])
            ...
            <div className="w-full h-full flex items-center justify-center">
                <div className="text-center max-w-md">
                    <Empty>
                        ...
                        <EmptyContent>
                            <EmptyTitle>Niciun an universitar creat</EmptyTitle>
                            <EmptyDescription>Începe prin a crea primul an universitar pentru a putea gestiona orarul</EmptyDescription>
                            <AdminOnlyServer>
                                <CreateYearModal
                                    trigger={
                                        <Button variant="brand" size="L"><CirclePlus /> Creează an universitar</Button>
                                    }
                                />
                            </AdminOnlyServer>
                        </EmptyContent>
    ...
    return (
        <div className="content grid grid-rows-[auto_1fr] h-full overflow-hidden">
            {/* Header - Fixed */}
            <div className="flex flex-col items-center bg-primary-200 border-b border-primary-300 p-6">
                <div className="flex flex-col gap-2 w-full max-w-7xl">
                    <H1 className="text-left text-2xl">Orar</H1>
                    <P className="text-lg [&:not(:first-child)]:mt-0">Selectați un an universitar pentru a vizualiza și gestiona orarul</P>
                </div>
            </div>

            {/* Scrollable Content */}
            <div className="flex-1 overflow-y-auto">
                <div className="w-full flex flex-col items-center gap-8 py-8 px-6">
                    <div className="flex justify-between w-full max-w-7xl">
                        <ul className="grid grid-cols-[repeat(auto-fill,minmax(320px,1fr))] auto-rows-min gap-6 w-full">
                            {years.map((year) => (
                                <li key={year.id} className="min-h-[120px]">

                                    <Card className="bg-primary-100 hover:bg-brand-400/20 hover:border-brand-400 transition-colors relative h-full overflow-hidden group py-0">
                                        ...
                                        <CardContent className="px-0 h-full">
                                            <div className="py-6 px-6 h-full flex flex-col justify-between">
                                                <H2 className="text-xl transition-transform duration-300">
                                                    <Link href={`/orar/${year.start}-${year.end}`}>
                                                        {year.start} - {year.end}
                                                    </Link>
                                                </H2>
                                                <div className="relative h-7 overflow-hidden">
                                                    ...
                                                    <P className="absolute inset-0 transition-all duration-300 group-hover:opacity-0 group-hover:-translate-y-full">
                                                        <span className="text-l">{year.events.length}</span> Evenimente
                                                    </P>

                                                    {/* Learning Types (Licență/Master) - Slide up și fade in */}
                                                    <div className="absolute inset-0 flex items-center gap-8 translate-y-full opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 ease-out">
                                                        {learningTypes.map((lt) => (
                                                            <Link
                                                                key={lt.id}
                                                                href={`/orar/${year.start}-${year.end}/${lt.learningCycle.toLowerCase()}`}
                                                                className="inline-flex items-center gap-1 text-brand-400 hover:text-brand-500 transition-colors"
                                                            >
                                                                {lt.learningCycle}
                                                            </Link>
                                                        ))}
                            ...
                            <AdminOnlyServer>
                                <li key="year-create" className="min-h-[120px]">
                                    <CreateYearModal
                                        trigger={
                                            <Card className="bg-primary-100 h-full p-0 hover:bg-brand-400/20 hover:border-brand-400 transition-colors cursor-pointer">
                                                <CardContent className="h-full flex items-center justify-center p-0">
                                                    <Button className="flex-col py-8 px-6 w-full h-full text-base" variant="text" size="L">
                                                        <CirclePlus className="size-6" /> Creează an universitar
                                                    </Button>
                                                </CardContent>
                                            </Card>
                                        }
