...
import { importAdmins } from "@/actions/import"
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (session.user.role !== "ADMIN") {
            return NextResponse.json(
                { error: "Nu ai permisiunea să imporți administratori" },
                { status: 403 }
            )
        }
        ...
        // Importăm administratorii
        const result = await importAdmins(data, session.user.id)

        return NextResponse.json(result)
    } catch (error) {
        console.error("Eroare la importul administratorilor:", error)
        return NextResponse.json(
            {
