...
import { StudentListWithSelection } from "./_components/StudentListWithSelection"
import { isAdminOrSecretarServer } from "@/lib/auth-server"
...
export default async function StudentsPage({ searchParams }: StudentsPageProps) {
    const [students, groups, canManage] = await Promise.all([
        getAllStudents(),
        getAllGroups(),
        isAdminOrSecretarServer()
    ])
    ...
    // Prepare student data for client component
    const studentsForSelection = filteredStudents.map((student) => ({
        id: student.id,
    ...
    // Prepare groups data for client component
    const groupsForSelection = groups.map((group) => ({
        id: group.id,
        name: group.name,
        group: group.group,
        semester: group.semester,
        studyYear: group.studyYear ? {
            id: group.studyYear.id,
            year: group.studyYear.year,
        } : null,
        learningType: group.learningType ? {
            id: group.learningType.id,
            learningCycle: group.learningType.learningCycle,
        } : null,
    }))

    // Create modal trigger for the add student card
                    ...
                    <div className="flex flex-col gap-8 w-full">
                        <Suspense fallback={<CardListSkeleton />}>
                            {filteredStudents.length === 0 && searchQuery ? (
                                ...
                                <StudentListWithSelection
                                    students={studentsForSelection}
                                    groups={groupsForSelection}
                                    canManage={canManage}
                                    createModal={
                                        <CreateStudentModal trigger={createStudentTrigger} />
                                    }
                                    cardActions={Object.fromEntries(
                                        filteredStudents.map((student) => [
                                            student.id,
                                            <CardActions key={student.id} studentId={student.id} />
                                        ])
                                    )}
                                />
                            )}
